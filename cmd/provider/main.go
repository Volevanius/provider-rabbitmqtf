...
import (
	...
	apisCluster "github.com/Volevanius/provider-rabbitmqtf/apis/cluster"
	apisNamespaced "github.com/Volevanius/provider-rabbitmqtf/apis/namespaced"
	"github.com/Volevanius/provider-rabbitmqtf/config"
	"github.com/Volevanius/provider-rabbitmqtf/internal/clients"
	controllerCluster "github.com/Volevanius/provider-rabbitmqtf/internal/controller/cluster"
	controllerNamespaced "github.com/Volevanius/provider-rabbitmqtf/internal/controller/namespaced"
	"github.com/Volevanius/provider-rabbitmqtf/internal/features"
	"github.com/Volevanius/provider-rabbitmqtf/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for RabbitMQTF").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-rabbitmqtf"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-rabbitmqtf",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped RabbitMQTF APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced RabbitMQTF APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped RabbitMQTF controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced RabbitMQTF controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped RabbitMQTF controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced RabbitMQTF controllers")
	}
